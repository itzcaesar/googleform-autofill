...
def choose_checkbox(options: List[str], entry_name: str,
    ...
    Real people mostly know 1-2 tools; engaged users up to 3-4. Uses a
    beta-skewed count + popularity-weighted sampling so popular tools repeat
    across respondents instead of every row being a unique combination.
    """
    r = rng if rng is not None else random
    n = len(options)
    if n == 0:
...
def human_delay(base: float, rng=None) -> float:
    ...
    plus a ~10% chance of a longer "reading/distracted" pause.
