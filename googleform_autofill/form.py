...
# Short-link hosts that redirect to a full docs.google.com form URL
SHORT_URL_HOSTS = ("forms.gle", "goo.gl")


def expand_short_url(url: str) -> str:
    """
    Resolve a Google Form short link (e.g. https://forms.gle/XXXX) to its
    full docs.google.com URL by following the redirect.

    Returns the original URL unchanged if it is not a short link or if the
    request fails.
    """
    try:
    ...
    if not any(h in host for h in SHORT_URL_HOSTS):
        return url
    ...
    try:
        ...
        if final and final != url:
            logger.info(f"Resolved short URL to: {final}")
            return final
...
def normalize_form_url(url: str) -> str:
    """
    Convert any Google Form URL (edit, viewform, short link, etc.) to viewform URL.

    Args:
        url: The Google Form URL (edit, viewform, forms.gle short link, or form ID)
    ...
    # Expand short links (forms.gle / goo.gl) to the full URL first
...
def parse_form_entries(url: str, only_required: bool = False) -> Optional[List[Dict[str, Any]]]:
    ...
    def parse_entry(entry: List[Any]) -> List[Dict[str, Any]]:
        ...
        # Some elements (section headers, images, text blocks) have no sub-entries
        if not entry[4]:
            return []
